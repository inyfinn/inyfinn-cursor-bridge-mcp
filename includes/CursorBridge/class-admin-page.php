<?php
/**
 * WP Admin: Cursor Bridge — health dashboard, repair, connection settings.
 *
...
final class Admin_Page {
	...
	public static function handle_actions(): void {
		...
		if ( isset( $_GET['inyfinn_repair'] ) && check_admin_referer( 'inyfinn_repair' ) ) {
			...
			if ( ! empty( $result['ok'] ) || ! empty( $result['health']['healthy'] ) ) {
				...
				add_settings_error( 'inyfinn_cursor_bridge', 'repair', $msg, 'error' );
			}
		}
	}
	...
	public static function render_page(): void {
		...
		$docs_base     = INYFINN_CURSOR_BRIDGE_MCP_DIR . 'docs/';
		...
		<div class="wrap">
			...
			<p>
				...
				printf(
					...
					esc_html__( 'Wersja %s — połączenie WordPress z Cursor IDE przez MCP.', 'inyfinn-cursor-bridge-mcp' ),
			...
			<h2><?php esc_html_e( 'Diagnostyka', 'inyfinn-cursor-bridge-mcp' ); ?></h2>
			<p><?php esc_html_e( 'Każdy wiersz to jeden warunek poprawnej pracy. Przy błędzie użyj przycisku Napraw.', 'inyfinn-cursor-bridge-mcp' ); ?></p>
			...
			<p style="margin-top:1em">
				<a href="<?php echo esc_url( $bootstrap_url ); ?>" class="button button-primary">
					<?php esc_html_e( 'Pełny auto-setup (wszystko od nowa)', 'inyfinn-cursor-bridge-mcp' ); ?>
				</a>
				<a href="<?php echo esc_url( self::repair_url( 'full_bootstrap' ) ); ?>" class="button">
					<?php esc_html_e( 'Napraw wszystko (bez rotacji hasła)', 'inyfinn-cursor-bridge-mcp' ); ?>
				</a>
			</p>

			<h2><?php esc_html_e( 'Jak sprawdzić, że działa (nowa instalacja)', 'inyfinn-cursor-bridge-mcp' ); ?></h2>
			<div class="card" style="max-width:960px;padding:1em 1.2em">
				<ol>
					<li><?php esc_html_e( 'Aktywuj wtyczkę w WP → Wtyczki.', 'inyfinn-cursor-bridge-mcp' ); ?></li>
					<li><?php esc_html_e( 'Wejdź tutaj (Ustawienia → Cursor Bridge) — wszystkie testy powinny być zielone.', 'inyfinn-cursor-bridge-mcp' ); ?></li>
					<li><?php esc_html_e( 'Otwórz folder public_html w Cursorze jako workspace (SFTP).', 'inyfinn-cursor-bridge-mcp' ); ?></li>
					<li>
						<?php esc_html_e( 'W chacie Cursora napisz dokładnie:', 'inyfinn-cursor-bridge-mcp' ); ?>
						<p><code style="font-size:14px;background:#f0f0f1;padding:8px 12px;display:inline-block">uruchom wtyczkę inyfinn-cursor-bridge-mcp</code></p>
					</li>
					<li>
						<?php esc_html_e( 'Po skonfigurowaniu MCP w Cursorze sprawdź:', 'inyfinn-cursor-bridge-mcp' ); ?>
						<ul style="list-style:disc;margin-left:1.5em">
							<li><code>cursor-bridge/ping</code> → <code>ok: true</code>, <code>bridge_version</code> = <?php echo esc_html( $health['version'] ?? '' ); ?></li>
							<li><code>cursor-bridge/health-check</code> → <code>healthy: true</code></li>
							<li>discover-abilities → min. 19 pozycji <code>cursor-bridge/*</code></li>
						</ul>
					</li>
				</ol>
				<p><strong>MCP endpoint:</strong> <code><?php echo esc_html( $health['mcp_endpoint'] ?? '' ); ?></code></p>
			</div>
			...
			<ul>
				<li><code>README.md</code> — <?php esc_html_e( 'przegląd i szybki start', 'inyfinn-cursor-bridge-mcp' ); ?></li>
				<li><code>docs/INSTALLATION.md</code> — <?php esc_html_e( 'instalacja krok po kroku', 'inyfinn-cursor-bridge-mcp' ); ?></li>
				<li><code>docs/TROUBLESHOOTING.md</code> — <?php esc_html_e( 'rozwiązywanie problemów', 'inyfinn-cursor-bridge-mcp' ); ?></li>
				<li><code>docs/ABILITIES.md</code> — <?php esc_html_e( 'pełna lista abilities MCP', 'inyfinn-cursor-bridge-mcp' ); ?></li>
				<li><code>AGENTS.md</code> — <?php esc_html_e( 'instrukcja dla agenta Cursor', 'inyfinn-cursor-bridge-mcp' ); ?></li>
			</ul>
		</div>
		<?php
	}
	...
	private static function render_health_banner( array $health ): void {
		...
		if ( 'ok' === $overall ) {
			...
			esc_html_e( 'Wszystko działa — wtyczka gotowa dla Cursor IDE.', 'inyfinn-cursor-bridge-mcp' );
			echo '</strong></p></div>';
			return;
		}
		...
		printf(
			...
			esc_html__( 'Wykryto problemy: %1$d błędów, %2$d ostrzeżeń. Użyj przycisków Napraw poniżej.', 'inyfinn-cursor-bridge-mcp' ),
