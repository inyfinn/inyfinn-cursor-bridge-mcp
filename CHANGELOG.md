...
### Fixed
- Narzędzia plików (`read-wp-content-file`, `list-wp-content-dir`, `write-wp-content-file`) wymagały `edit_themes` / `edit_plugins`. WordPress odbiera te uprawnienia adminowi, gdy `wp-config.php` ma `DISALLOW_FILE_EDIT` — agent dostawał gołe „Permission denied”, a na hostingu bez FTP (kubara) nie miał żadnej drogi do plików. Teraz odczyt i listowanie wymagają konta administratora.
- **`DISALLOW_FILE_EDIT` wyłączane przez wtyczkę** (decyzja właściciela): przy instalacji, aktualizacji albo `repair {action:"file_edit"}` wtyczka robi kopię `wp-config.php`, zakomentowuje `define( 'DISALLOW_FILE_EDIT', … )` z datą i opisem (nie kasuje), sprawdza składnię i przy problemie przywraca kopię. Działa od następnego żądania. Stała ustawiona poza `wp-config.php` → komunikat, gdzie jej szukać, i przełącznik w Ustawienia → Cursor Bridge („Zapis plików przez MCP mimo DISALLOW_FILE_EDIT”) albo stała `INYFINN_BRIDGE_ALLOW_FILE_WRITE`. Uwaga: przywraca to też edytor plików w panelu dla administratorów.
- Test bez WordPressa: `php tests/file-edit-check.php` (9 wariantów zapisu stałej).
...
### Added
- **`docs/ODTWORZENIE.md`** — pełna instrukcja odtworzenia projektu od zera (WP + baza + pliki + Cursor)
- Link w README do instrukcji odtworzenia
